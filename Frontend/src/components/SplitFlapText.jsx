...
const SplitFlapText = ({
  ...
  useEffect(() => {
    ...
    const animateTo = targetPhrase => {
      ...
      const plans = targetPhrase.split('')
      ...
      const totalDuration = plans.reduce(
        (max, plan) => Math.max(max, plan.start + plan.sequence.length * safeFlipMs), 0
      );
      ...
      const updateTiles = updates => {
        setTiles(prev => {
          ...
          updates.forEach(u => {
            ...
            if (!t) return;
            next[u.index] = { current: u.current, next: u.next, flipping: !u.done, tick: t.tick + 1 };
          });
          return next;
        });
      };
    ...
    const scheduleNext = delay => {
      cycleTimerRef.current = window.setTimeout(() => {
        ...
        const animDur = animateTo(normalizedPhrases[phraseIndex]);
        scheduleNext(safeCycleDelay + animDur);
      }, delay);
    };
